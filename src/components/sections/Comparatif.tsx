...
const SCORES = [
  ...
  { label: "Sage",   key: "sage"   as const, color: "rgba(232,237,244,0.35)", highlight: false },
  { label: "EBP",    key: "ebp"    as const, color: "rgba(232,237,244,0.25)", highlight: false },
];
...
function Cell({ val, isFloxia }: { val: Val; isFloxia?: boolean }) {
  ...
  if (val === false)     return <X size={15} color="rgba(232,237,244,0.15)" style={{ margin: "0 auto" }} />;
  if (val === "partial") return <Minus size={15} color="rgba(232,237,244,0.32)" style={{ margin: "0 auto" }} />;
  return <span style={{ color: isFloxia ? "#F5C842" : "rgba(232,237,244,0.45)", fontWeight: isFloxia ? 700 : 400, fontSize: ".8rem" }}>{val}</span>;
}
    ...
    <section style={{ background: "#0A1520", padding: "6rem 0" }}>
      <div style={{ maxWidth: "80rem", margin: "0 auto", padding: "0 6vw" }}>
          ...
          {SCORES.map((s, i) => {
            ...
            return (
              <motion.div key={s.label} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.08 }}
                style={{ background: s.highlight ? "rgba(245,200,66,0.07)" : "rgba(255,255,255,0.02)", border: `1px solid ${s.highlight ? "rgba(245,200,66,0.25)" : "rgba(255,255,255,0.06)"}`, borderRadius: "1rem", padding: "1.1rem 1.25rem", display: "flex", alignItems: "center", gap: "0.85rem" }}>
                <div style={{ flex: 1 }}>
                  <p style={{ color: s.highlight ? "#F5C842" : "rgba(232,237,244,0.5)", fontWeight: 700, fontSize: ".88rem" }}>{s.label}</p>
                  <div style={{ marginTop: "0.4rem", height: 4, borderRadius: 999, background: "rgba(255,255,255,0.06)", overflow: "hidden" }}>
                ...
                <div style={{ textAlign: "right", flexShrink: 0 }}>
                  ...
                  <span style={{ color: "rgba(232,237,244,0.25)", fontSize: ".72rem" }}>/{TOTAL}</span>
                </div>
              </motion.div>
            );
          })}
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}
          style={{ borderRadius: "1.25rem", overflow: "hidden", border: "1px solid rgba(255,255,255,0.07)" }}>
          <div style={{ display: "grid", gridTemplateColumns: GRID, background: "rgba(255,255,255,0.04)", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
            <div style={{ padding: "1rem 1.5rem", color: "rgba(232,237,244,0.3)", fontSize: ".72rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".1em" }}>Critère</div>
            {COLS.map(col => (
              <div key={col.name} style={{ padding: "1rem 0.75rem", textAlign: "center", background: col.highlight ? "rgba(245,200,66,0.08)" : "transparent", borderLeft: `1px solid ${col.highlight ? "rgba(245,200,66,0.2)" : "rgba(255,255,255,0.05)"}`, color: col.highlight ? "#F5C842" : "rgba(232,237,244,0.4)", fontWeight: 700, fontSize: ".82rem" }}>
                {col.name}
              </div>
            ))}
          </div>
          ...
          {SECTIONS.map((section, si) => (
            <div key={section.label}>
              <div style={{ display: "grid", gridTemplateColumns: GRID, background: "rgba(255,255,255,0.02)", borderTop: si > 0 ? "2px solid rgba(255,255,255,0.07)" : undefined }}>
                ...
                {COLS.map((col, j) => (
                  <div key={j} style={{ background: col.highlight ? "rgba(245,200,66,0.05)" : "transparent", borderLeft: `1px solid ${col.highlight ? "rgba(245,200,66,0.15)" : "rgba(255,255,255,0.04)"}` }} />
                ))}
              </div>
              {section.rows.map((row, i) => (
                <div key={row.feature} style={{ display: "grid", gridTemplateColumns: GRID, background: i % 2 === 0 ? "rgba(255,255,255,0.01)" : "transparent", borderTop: "1px solid rgba(255,255,255,0.04)" }}>
                  <div style={{ padding: "0.8rem 1.5rem", color: "rgba(232,237,244,0.78)", fontSize: ".84rem", display: "flex", alignItems: "center" }}>{row.feature}</div>
                  {COLS.map((col, j) => (
                    <div key={j} style={{ padding: "0.8rem 0.75rem", textAlign: "center", display: "flex", alignItems: "center", justifyContent: "center", background: col.highlight ? "rgba(245,200,66,0.04)" : "transparent", borderLeft: `1px solid ${col.highlight ? "rgba(245,200,66,0.12)" : "rgba(255,255,255,0.04)"}` }}>
        ...
        <p style={{ marginTop: "1rem", textAlign: "center", color: "rgba(232,237,244,0.22)", fontSize: ".7rem" }}>
          * Prix Sage/EBP hors modules bâtiment additionnels &nbsp;·&nbsp; <span style={{ color: "rgba(232,237,244,0.32)" }}>— Partiel</span> = option payante ou fonctionnalité limitée
        </p>
      </div>
    </section>
  );
