...
const GhostCursor = ({
  ...
  // Styles du composant en inline + <style> local (le projet n'utilise pas Tailwind
  // ni de fichiers CSS par composant — cf. AGENTS.md).
  const mergedStyle = useMemo(
    () => ({ position: 'absolute', inset: 0, pointerEvents: 'none', zIndex, ...style }),
    [zIndex, style]
  );

  return (
    <>
      <style>{`.ghost-cursor > canvas { display: block; width: 100%; height: 100%; background: transparent; }`}</style>
      <div ref={containerRef} className={`ghost-cursor ${className ?? ''}`} style={mergedStyle} aria-hidden />
    </>
  );
};
