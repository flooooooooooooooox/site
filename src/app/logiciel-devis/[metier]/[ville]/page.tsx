...
export async function generateMetadata({ params }: { params: Promise<{ metier: string; ville: string }> }): Promise<Metadata> {
  ...
  const v = getVille(vs);
  if (!m || !v) return {};
  const url = `https://florianai.fr/logiciel-devis/${m.slug}/${v.slug}`;
...
export default async function MetierVillePage({ params }: { params: Promise<{ metier: string; ville: string }> }) {
  ...
  const breadcrumb = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Logiciel de devis", item: "https://florianai.fr/logiciel-devis" },
      { "@type": "ListItem", position: 3, name: m.label, item: `https://florianai.fr/artisans/${m.slug}` },
